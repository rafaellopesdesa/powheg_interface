...
c pdfb: born pdf's, pdfs: pdfs with scaled x->x/z
      real * 8 pdfb1(-nparton:nparton),pdfb2(-nparton:nparton),
     1         pdfs1(-nparton:nparton),pdfs2(-nparton:nparton),
     2         pdfs1sng,pdfs2sng
      real * 8 z,omzpgg,omzpqq,ppqq,omzpqg,ppqg,omzpgq,ppgq,
     1         sb,tot,plfrc,plfr0,z1,z2,xjac1,xjac2,rm1,rm2,res1,res2,
     2         x,xjac
      integer j,jb,fl1,fl2,flemitted
...
c WEW, add photon remnants       
         rescoll(jb)=rescoll(jb)+(resgam1*pdfb2(fl2)+resgam2*pdfb1(fl1))
     #    *br_born(jb)*physpar_aem/(2*pi)*kn_jacborn
         tot=tot+rescoll(jb)
      enddo
