...
c     check parameters
      ...
      logical check_z, check_e, check_p
...
c      endif
      ...
      check_z = .false.
      do ipart=1, nhep
       if ((isthep(ipart).eq.1).and.(idhep(ipart).eq.11)) then
          imo = jmohep(1, ipart)
          do while (idhep(jmohep(1, imo)).eq.11)
             imo = jmohep(1, imo)
          enddo
          imo = jmohep(1, imo)
          if (abs(idhep(imo)).eq.23) then
...
C             write(*,*) 'Found the electron'
       ...
       else if ((isthep(ipart).eq.1).and.(idhep(ipart).eq.-11)) then
          imo = jmohep(1, ipart)
          do while (idhep(jmohep(1, imo)).eq.-11)
             imo = jmohep(1, imo)
          enddo
...
C             write(*,*) 'Found the neutrino'
       ...
       else if ((isthep(ipart).eq.2).and.(abs(idhep(ipart)).eq.23)) then
...
c          write(*,*) 'Found the W'
      ...
      enddo
      if (.not.(check_e.and.check_p.and.check_z)) then
         write(*,*) 'Missing'
         write(*,*) 'Electron, Positron, Z', check_e, check_p, check_z
      endif
...
c      endif
      ...
      write(15,*) 0., 0., 0.
      write(15,*) id_parts(1),
     & partvec2(1, 1), partvec2(2, 1), 
     & partvec2(3, 1), partvec2(4, 1),
     & '0 0'
      do ipart=2, nphotos
